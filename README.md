...
| Media is not a valid zip archive | Oracle archive downloads may have redirected to a license or login page instead of returning the APEX/ORDS zip. | Open the Oracle download page in a browser, accept the license, download the authorized zip, then place it at the reported media path or rerun `install.sh` with an absolute local media path. |
